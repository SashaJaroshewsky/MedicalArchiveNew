...
import { Vaccination, VaccinationUpdateRequest } from '../../models/vaccination.model';
...
const VaccinationDetailPage: React.FC = () => {
  ...
  const loadVaccination = async () => {
    try {
      setLoading(true);
      const data = await VaccinationsAPI.getVaccinationById(Number(id));
      setVaccination(data);
      setError(null);
    } catch (err) {
  ...
  const handleUpdate = async (data: VaccinationUpdateRequest) => {
    try {
      setIsSubmitting(true);
      setFormError(null);
      await VaccinationsAPI.updateVaccination(Number(id), data);
  ...
  const handleDelete = async () => {
    ...
    try {
      await VaccinationsAPI.deleteVaccination(Number(id));
      ...
      <Layout>
        <Container fluid className="py-4">
          <Alert variant="danger">
            Запис про вакцинацію не знайдено
          </Alert>
          <Link to="/vaccinations" className="btn btn-primary">
  ...
  return (
    <Layout>
            ...
            <Card className="shadow-sm">
              <Card.Body>
                <div className="d-flex justify-content-between align-items-center mb-4">
                  <div>
                    <h1 className="h3 mb-0">{vaccination.vaccineName}</h1>
                    <p className="text-muted mb-0">
                      Дата вакцинації: {format(new Date(vaccination.vaccinationDate), 'dd.MM.yyyy')}
                    </p>
                  </div>
                ...
                <Row className="g-4">
                  <Col md={6}>
                    <Card className="h-100">
                      <Card.Body>
                        <h5 className="card-title mb-3">Інформація про вакцинацію</h5>
                        <dl className="mb-0">
                          <dt>Назва вакцини</dt>
                          <dd className="mb-3">{vaccination.vaccineName}</dd>
                          
                          <dt>Виробник</dt>
                          <dd className="mb-3">{vaccination.manufacturer}</dd>
                          
                          <dt>Номер дози</dt>
                          <dd className="mb-3">{vaccination.doseNumber}</dd>
                        </dl>
                      </Card.Body>
                    </Card>
                  </Col>
                        ...
                        {vaccination.documentFilePath ? (
                          <>
                            ...
                            <div className="document-preview">
                              {renderDocument(vaccination.documentFilePath)}
                            </div>
                          </>
                        ) : (
      ...
      <Modal
        ...
        title="Редагування запису про вакцинацію"
        size="lg"
        submitButton={{
          text: "Зберегти зміни",
